...
export function checkMinRest(existingAssignments: Assignment[], newShift: Shift): Violation | null {
  ...
  const previousShift = sortedAssignments.find((a) => dayjs(a.shift.endTime).isBefore(newStart));

  if (previousShift) {
    const gapHours = dayjs(newStart).diff(dayjs(previousShift.shift.endTime), 'hour');
  ...
  const nextShift = sortedAssignments.find((a) => dayjs(a.shift.startTime).isAfter(newEnd));
