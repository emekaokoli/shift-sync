...
export function isPremiumShift(shift: Shift): boolean {
  ...
  const timezone = (shift as unknown as { location?: { timezone: string } }).location?.timezone;
...
export function calculateFairnessScore(
  ...
  const expectedShare = totalPremium.length > 0 ? totalPremium.length / allShifts.length : 0;
  const actualShare = totalShifts > 0 ? premiumShifts.length / Math.max(1, allShifts.length) : 0;
